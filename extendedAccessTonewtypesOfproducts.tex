...
SELECT * FROM ivoa.obscore
 NATURAL JOIN ivoa.obscore_hea
 JOIN datalink.response ON
 ivoa.obscore.response_id = datalink.response.dl_association_id
 WHERE (INTERSECTS(s_region, CIRCLE(312.775, 30.683, 1.5)) = 6)
 AND (dataproduct_type = ’hea-event-list’)
 AND (semantics = '#this')
\end{verbatim}
...
\caption{Example Response Table.  With further scientific input from \gls{HEIG} domain experts, the columns identified herein could form the basis for a recommended base set of columns for any {\tt ivoa.response$\{$\_xxx$\}$} table.}
